...
def get_category_id(category_name):
    ...
    response = requests.get(f'https://api.moltin.com/v2/categories?filter=eq(slug,{category_name})', headers=headers)
    response.raise_for_status()
...
def get_products_by_category(category_name='main'):
    headers = {
        'Authorization': f'Bearer {get_access_token()}',
    }
